...
import { PostDataType, PostViewType } from '../types/post';
...
export class PostRepository {
  ...
  //(1) method returns all posts
  async findAll(
    searchNameTerm: string, // might be empty
    sortBy: string,
  ...
  //(2) count of all posts
  async countAllPosts(blogId: string, searchNameTerm: any): Promise<number> {
    const filter = blogId
      ? searchNameTerm
        ? { $and: [{ blogId: blogId }, { title: { $regex: searchNameTerm, $options: 'i' } }] }
        : { blogId: blogId }
  ...
  //(5) method updates post by ID
  async updatePostById(postId: string, dto: PostDTO): Promise<number> {
